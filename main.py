...
def _load_pending_deps() -> dict:
    try:
        with open(_PENDING_DEPS_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}

def _save_pending_deps(data: dict):
    with open(_PENDING_DEPS_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)

def _load_rejected_deps() -> list:
    try:
        with open(_REJECTED_DEPS_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return []

def _save_rejected_deps(data: list):
    with open(_REJECTED_DEPS_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
...
def _load_manual_orders() -> dict:
    try:
        with open(_MANUAL_ORDERS_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}

def _save_manual_orders(data: dict):
    with open(_MANUAL_ORDERS_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    ...
    if os.path.exists(MARKET_FILE):
        try:
            with open(MARKET_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            data.setdefault("vpn_durations", _DEFAULT_VPN_DURS)
            ...
            pass
    return {"categories": {}, "products": {}, "vpn_durations": dict(_DEFAULT_VPN_DURS)}
...
def get_stock_count(p_name):
    ...
    fp = p_info.get("file", "")
    if not fp or not os.path.exists(fp): return 0
    try:
        return len(pd.read_excel(fp))
    except Exception:
        return 0
    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            for uid in data:
                ...
                if not data[uid].get("language"):
                    data[uid]["language"] = "bn"
            return data
        except Exception:
            return {}
    return {}
...
def _deliver_mail_text(uid, p_name, qty, total, S):
    """Retail delivery: read Column A (from row 2), send as text (one account per line)."""
    fp  = get_products()[p_name]["file"]
    df  = pd.read_excel(fp, header=0, usecols=[0])   # Column A only
    col = df.iloc[:, 0]                               # Series of all rows
    available = col.dropna().tolist()
    if len(available) < qty:
        raise ValueError(f"Not enough stock: have {len(available)}, need {qty}")
    to_send   = available[:qty]
    remaining = col.iloc[qty:]
    # Write remaining back — preserve full df but drop used rows
    full_df   = pd.read_excel(fp, header=0)
    full_df.iloc[:qty, 0] = None          # blank out used rows in col A
    full_df   = full_df.dropna(subset=[full_df.columns[0]])   # drop rows where col A is NaN
    full_df.to_excel(fp, index=False)
...
def _deliver_mail_xlsx(uid, p_name, qty, total, S):
    ...
    to_send = df.head(qty)
    remaining = df.iloc[qty:]
    remaining.to_excel(fp, index=False)
    out = io.BytesIO()
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        to_send.to_excel(writer, index=False)
    out.seek(0)
...
def _deliver_mail(uid, call, p_name, qty, total, success_msg, S):
    """Deliver mail accounts as .xlsx from local stock."""
    fp        = get_products()[p_name]["file"]
    df        = pd.read_excel(fp, header=0)
    to_send   = df.head(qty)
    remaining = df.iloc[qty:]
...
def save_xlsx(message, p_name):
    ...
    prods     = get_products(); file_info = bot.get_file(message.document.file_id)
    downloaded = bot.download_file(file_info.file_path)
    file_path = prods[p_name]["file"]
    with open(file_path, "wb") as f: f.write(downloaded)
    # Clear synced_stock so we read from xlsx
    ...
    stock = get_stock_count(p_name)
    bot.send_message(ADMIN_ID, f"✅ *{p_name}* স্টক আপডেট! 📦 মোট: *{stock}* পিস", parse_mode="Markdown")
...
def do_restore(message):
    ...
    with open(DATA_FILE, "wb") as f: f.write(bot.download_file(f_info.file_path))
    global user_data; user_data = load_data()
    bot.send_message(ADMIN_ID, "✅ রিস্টোর সফল!")
...
def _do_backup():
    ...
    for filepath, caption in [
        (DATA_FILE, "📂 User Data"), (MARKET_FILE, "📦 Market Data"),
        (SETTINGS_FILE, "⚙️ Settings"), (TEXTS_FILE, "📝 Texts"), (COUPONS_FILE, "🎟️ Coupons"),
    ]:
        if os.path.exists(filepath):
            with open(filepath, "rb") as f:
                try: bot.send_document(ADMIN_ID, f, caption=caption)
                except Exception: pass
